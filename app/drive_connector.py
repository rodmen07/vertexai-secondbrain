...
ndef _build_service(credentials: Optional[Any] = None):
...
nclass DriveConnector:
    """Minimal Drive connector.

    Methods are synchronous and return plain Python objects for easy testing.
    """
n    SCOPES = ["https://www.googleapis.com/auth/drive.readonly"]
n    def __init__(self, credentials: Optional[Any] = None):
        self.credentials = credentials
n    def list_files(self, q: Optional[str] = None, page_size: int = 100) -> List[Dict[str, Any]]:
        """List files in Drive.
n        Returns a list of file dicts with `id` and `name` keys when available.
        """
...
n    def download_file(self, file_id: str) -> bytes:
        """Download the raw bytes of a file by id.
n        This implementation uses `files().get_media(...).execute()` which is
